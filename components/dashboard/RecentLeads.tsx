...
export const RecentLeads = React.memo(function RecentLeads({ leads, settings }: { leads: any[]; settings?: any }) {
    return (
        <Card className="col-span-1 lg:col-span-3 rounded-3xl border-white/10 bg-card/40 backdrop-blur-xl shadow-xl overflow-hidden self-start">
            ...
            <CardContent>
                <div className="space-y-2">
                    {leads.map((lead) => {
                        const statusObj = settings?.statuses?.find((s: any) => s.key === lead.status);
                        return (
                            <Link
                                key={lead._id}
                                href={`/leads/${lead._id}`}
                            ...
                            >
                                ...
                                <div className="ml-auto flex flex-col items-end gap-1 shrink-0">
                                    ...
                                    >
                                        {statusObj?.emoji && <span className="mr-0.5">{statusObj.emoji}</span>}
